...
public class AddVisitCommand : PatientCommand 
{
    ...
    public void Undo()
    {
        patientvisitmanageobj.DeleteVisit(patientvisitobj.Id);
    }
}
...
public class PatientVisitManager
{
    ...
    private void loadFeeRates()
    {
        ...
        try
        {
            ...
            {
                feeRates["Consultation"] = 500;
                feeRates["Follow-up"] = 300;
                feeRates["Emergency"] = 1000;
                feeRates["Checkup"] = 400;
                feeRates["Surgery"] = 5000;
                feeRates["Lab Test"] = 200;
                feeRates["X-Ray"] = 350;
    ...
    private bool checkTimeSlotConflict(string patientName, DateTime visitDate)
    {
        for (int i = 0; i < visits.Count; i++)
        {
            ...
            {
                TimeSpan timeDiff = Math.Abs((visits[i].VisitDate - visitDate).TotalMinutes) < 30 ? 
                                   visits[i].VisitDate - visitDate : TimeSpan.Zero;
    ...
    {
        PatientVisit updatevisitobj = null;

        for (int i = 0; i < visits.Count; i++)
        {
            if (visits[i].Id == id)
            {
                updatevisitobj = visits[i];
                break;
            }
        }

        if (updatevisitobj == null)
        {
        ...
        if (patientName != null) updatevisitobj.PatientName = patientName;
        if (visitDate.HasValue) updatevisitobj.VisitDate = visitDate.Value;
        if (visitType != null) updatevisitobj.VisitType = visitType;
        if (Note != null) updatevisitobj.Note = Note;
        if (doctorName != null) updatevisitobj.DoctorName = doctorName;
        if (duration.HasValue) 
        {
            updatevisitobj.DurationInMinutes = duration.Value;
            updatevisitobj.Fee = calculateFee(updatevisitobj.VisitType, duration.Value);
        }
    ...
    {
        PatientVisit deletevisitobj = null;

        for (int i = 0; i < visits.Count; i++)
        {
            if (visits[i].Id == id)
            {
                deletevisitobj = visits[i];
                break;
            }
        }

        if (deletevisitobj == null)
        {
    ...
    public List<PatientVisit> getVisitsByDoctor(string doctorName)
    {
        ...
        for (int i = 0; i < visits.Count; i++)
        {
            if (visits[i].DoctorName.ToLower().Contains(doctorName.ToLower()))
            {
                result.Add(visits[i]);
            }
        }
    ...
    public List<PatientVisit> getSortedVisits(string sortBy)
    {
        ...
        {
            sortedList.Sort((x, y) => x.VisitDate.CompareTo(y.VisitDate));
        }
        else if (sortBy.ToLower() == "name")
        {
            sortedList.Sort((x, y) => x.PatientName.CompareTo(y.PatientName));
        }
        else if (sortBy.ToLower() == "type")
        {
            sortedList.Sort((x, y) => x.VisitType.CompareTo(y.VisitType));
        }
        else if (sortBy.ToLower() == "fee")
        {
            sortedList.Sort((x, y) => x.Fee.CompareTo(y.Fee));
        }
...
public class Program
{
    ...
    private static bool authenticateUser()
    {
        ...
        if (roleChoice == "1" && username == "admin" && password == "admin123")
        {
            currentUserRole = UserRole.Admin;
            Console.WriteLine("Admin login successful!");
            return true;
        }
        else if (roleChoice == "2" && username == "receptionist" && password == "recep123")
        {
